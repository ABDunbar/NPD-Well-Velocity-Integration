...
class LASItem(object):
    ...
    def from_line(cls, line):                       # method called by LASReader
        ...
        return LASItem(name=name, units=units, data=data.strip(),
                       descr=descr.strip())       # return calls itself with parameters via "from_line" called by LASReader
                ...
                if line.startswith('~'):                   # The other (non Log) sections (~Version, ~Well, etc)
                    ...
                    if other:                               # "Other" section exists...
                        ...
                        if current_section == self.version:
                            if m.name == 'WRAP':            # e.g. Line 3, WRAP.  NO  :SINGLE LINE PER DEPTH STEP
                                if m.data.strip() == 'YES':
                                    self.wrap = True
                            if m.name == 'VERS':
                        ...
                        if current_section == self.well:
                            if m.name == 'NULL':
                                self.null = float(m.data)   # Store the NULL value
                            elif m.name == 'STRT':
                                self.start = float(m.data)  # Store the STRT value
                                self.start_units = m.units  # Store the units
                            elif m.name == 'STOP':
                                self.stop = float(m.data)   # Store the STOP value
                                self.stop_units = m.units   # Store the units
                            elif m.name == 'STEP':
                                self.step = float(m.data)   # Store the STEP value
                                self.step_units = m.units   # Store the units
            line = f.readline()
